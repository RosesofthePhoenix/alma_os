...
- Continuum Tracker: timer + quick-capture time entries, notes, and journals with per-entry EEG summaries; Analyze Patterns and Reflect/Synthesize buttons call mini-Oracle with recent note/journal text (mood + EEG context) and clarify HCE is an EEG-derived metric; agenda calendar and timelines include notes/journals alongside time entries.
