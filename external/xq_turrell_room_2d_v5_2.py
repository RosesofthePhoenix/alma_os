    ...
    class TurrellishRenderer:
        ...
        def render(
            ...
            if mode == MODE_MAX_FOCUS:
                return self._render_focus(x, q, gate, t, exposure, bloom_strength, bloom_thr, drift_speed, micro_texture_gain, warmth_bias)
            if mode == MODE_MAX_Q:
                return self._render_max_q(x, q, gate, t, exposure, bloom_strength, bloom_thr, drift_speed, micro_texture_gain, warmth_bias)
            if mode == MODE_STABILIZE:
                return self._render_stabilize(x, q, gate, t, exposure, bloom_strength, bloom_thr, drift_speed, micro_texture_gain, warmth_bias)
            # default / OFF
            return self._render_off(x, q, gate, t, exposure, bloom_strength, bloom_thr, drift_speed, micro_texture_gain, warmth_bias)
        ...
        def _render_off(self, x: float, q: float, gate: float, t: float, exposure: float, bloom_strength: float, bloom_thr: float, drift_speed: float, micro_texture_gain: float, warmth_bias: float) -> np.ndarray:
            # OFF = ambient neuro-reactive: fixed portal, hue/vibrancy from Q only
            sp = 0.05 * (0.30 + q) * drift_speed  # slow drift; no position motion
            wall, portal, rim = self.palette_q_triplet(q)

            # Fixed portal position/size (no wobble)
            cx = 0.0
            cy = -0.06
            ...
            halo = np.exp(-(np.maximum(d, 0.0) / (0.20 + 0.08 * q)) ** 2)
            rim_mask = np.exp(-(np.abs(d) / 0.010) ** 2)

            intensity = (0.35 + 1.40 * q) * gate

            lin += portal_mask[..., None] * portal[None, None, :] * (2.10 * intensity)
            lin += halo[..., None] * portal[None, None, :] * (0.55 * intensity)
            lin += rim_mask[..., None] * rim[None, None, :] * (2.55 * intensity)

            # subtle floor cue
            floor = smoothstep(0.10, 1.00, self.v)
            lin *= (1.0 - 0.08 * floor)[..., None]

            return self._apply_post(lin, exposure=exposure * 1.05, bloom_strength=bloom_strength * 1.10, bloom_thr=bloom_thr, micro_texture_gain=micro_texture_gain, warmth_bias=warmth_bias)
...
def main() -> int:
    ...
    while running:
        ...
        if toast_msg and (now < toast_until):
            ts = font.render(toast_msg, True, (235, 235, 235))
            screen.blit(ts, (14, sh - ts.get_height() - 14))
